...
void text( int sc)
{
	...
	//	renderBitmapString(0,0.5,20,GLUT_BITMAP_TIMES_ROMAN_24 ,"GAMEOVER");
		sprintf(text,"GAME OVERRR  SCORE::%d  ",sc);
	...
	if(start==0)
	{
			glPushMatrix();
	        glTranslatef(-1,0,0);
	       // PlaySound("starting.wav",NULL, SND_FILENAME|SND_ASYNC);
	
        glColor3f(1.000, 1.000, 0.000);
        renderBitmapString(0,2.5,20,GLUT_BITMAP_TIMES_ROMAN_24,"BRICK BREAKER ");

        glColor3f(0.000, 1.000, 0.000);
        renderBitmapString(0,0,20,GLUT_BITMAP_HELVETICA_18,"Press S to START");
        renderBitmapString(0,-0.2,20,GLUT_BITMAP_HELVETICA_18,"Press Q to Exit");

        glColor3f(1.000, 1.000, 1.000);
        renderBitmapString(-0.1,-0.4,20,GLUT_BITMAP_8_BY_13,"Press D to Move Right");
        renderBitmapString(-0.1,-0.6,20,GLUT_BITMAP_8_BY_13,"Press A to Move Left");
        renderBitmapString(-0.1,-0.7,20,GLUT_BITMAP_8_BY_13,"OR");
        renderBitmapString(-0.1,-0.9,20,GLUT_BITMAP_8_BY_13,"MOVE mouse to move paddle");


      //  glColor3f(0.000, 1.000, 1.000);
        renderBitmapString(2.5,-2.5,20,GLUT_BITMAP_8_BY_13,"Project By:");
        renderBitmapString(3.1,-2.5,20,GLUT_BITMAP_8_BY_13,"K.NARESH KUMAR & V.Hanmisha ");
        

	glPushMatrix();
	glTranslatef(-1,0,0);
	
	renderBitmapString(0,0.3,20,GLUT_BITMAP_HELVETICA_18 ,text);
	...
	glTranslatef(-1,0,0);
	//glRotatef(20,1,0,0);
	//if(c==1){renderBitmapString(0,0,20,GLUT_BITMAP_HELVETICA_18 ,end);}
//	else{
	
	renderBitmapString(0,0,20,GLUT_BITMAP_HELVETICA_18 ,text); 
	glEnable(GL_LIGHTING);
	glPopMatrix();
	glPopMatrix(); }
}
...
void idle()
{
	...
	if( by<=-12.8 && bx<(px+x*2/3) && bx>(px+x/3)&& start == 1 )
	{
		...
		PlaySound("paddle_sound.wav",NULL, SND_FILENAME|SND_ASYNC);
	}
